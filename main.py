...
#Creates a list of the differencce between values for the profits and losses
...
values = ["Total Months", "Total", "Average Change", "Greatest Increase in Profits", "Greatest Decrease in Profits"]
results = [(f'{len(int_profits_losses)}'),
...
output = list(zip(values, results))


output_text = os.path.join('PyBank', 'Resources', 'output_pybank.csv')

#Opening our output text file
with open(output_text, 'w') as csv_file:
